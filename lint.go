...
func (f *file) lintPackageComment() {
	...
	if f.f.Doc == nil {
		f.errorf(f.f, 0.2, link(ref), category("comments"), T("should have a package comment, unless it's in another file for this package"))
		return
	}
	s := f.f.Doc.Text()
	if ts := strings.TrimLeft(s, " \t"); ts != s {
		f.errorf(f.f.Doc, 1, link(ref), category("comments"), T("package comment should not have leading space"))
		s = ts
	}
	// Only non-main packages need to keep to this form.
	if !f.pkg.main && !strings.HasPrefix(s, prefix) {
		f.errorf(f.f.Doc, 1, link(ref), category("comments"), T(`package comment should be of the form "%s..."`), prefix)
	}
}
...
func (f *file) lintBlankImports() {
	...
	for i, imp := range f.f.Imports {
		...
		if imp.Doc == nil && imp.Comment == nil {
			ref := ""
			f.errorf(imp, 1, link(ref), category("imports"), T("a blank import should be only in a main or test package, or have a comment justifying it"))
		}
	}
}
...
func (f *file) lintImports() {
	for i, is := range f.f.Imports {
		...
		if is.Name != nil && is.Name.Name == "." && !f.isTest() {
			f.errorf(is, 1, link(styleGuideBase+"#import-dot"), category("imports"), T("should not use dot imports"))
		}
...
func (f *file) lintNames() {
	...
	if strings.Contains(f.f.Name.Name, "_") && !strings.HasSuffix(f.f.Name.Name, "_test") {
		f.errorf(f.f, 1, link("http://golang.org/doc/effective_go.html#package-names"), category("naming"), T("don't use an underscore in package name"))
	}
	...
	check := func(id *ast.Ident, thing string) {
		...
		if len(id.Name) >= 5 && allCapsRE.MatchString(id.Name) && strings.Contains(id.Name, "_") {
			f.errorf(id, 0.8, link(styleGuideBase+"#mixed-caps"), category("naming"), T("don't use ALL_CAPS in Go names; use CamelCase"))
			return
		}
		if len(id.Name) > 2 && id.Name[0] == 'k' && id.Name[1] >= 'A' && id.Name[1] <= 'Z' {
			should := string(id.Name[1]+'a'-'A') + id.Name[2:]
			f.errorf(id, 0.8, link(styleGuideBase+"#mixed-caps"), category("naming"), T("don't use leading k in Go names; %s %s should be %s"), thing, id.Name, should)
		}
		...
		if len(id.Name) > 2 && strings.Contains(id.Name[1:], "_") {
			f.errorf(id, 0.9, link("http://golang.org/doc/effective_go.html#mixed-caps"), category("naming"), T("don't use underscores in Go names; %s %s should be %s"), thing, id.Name, should)
			return
		}
		f.errorf(id, 0.8, link(styleGuideBase+"#initialisms"), category("naming"), T("%s %s should be %s"), thing, id.Name, should)
	}
...
func (f *file) lintTypeDoc(t *ast.TypeSpec, doc *ast.CommentGroup) {
	...
	if doc == nil {
		f.errorf(t, 1, link(docCommentsLink), category("comments"), T("exported type %v should have comment or be unexported"), t.Name)
		return
	}
	...
	if !strings.HasPrefix(s, t.Name.Name+" ") {
		f.errorf(doc, 1, link(docCommentsLink), category("comments"), T(`comment on exported type %v should be of the form "%v ..." (with optional leading article)`), t.Name, t.Name)
	}
}
...
func (f *file) lintFuncDoc(fn *ast.FuncDecl) {
	...
	if fn.Doc == nil {
		f.errorf(fn, 1, link(docCommentsLink), category("comments"), T("exported %s %s should have comment or be unexported"), kind, name)
		return
	}
	s := fn.Doc.Text()
	prefix := fn.Name.Name + " "
	if !strings.HasPrefix(s, prefix) {
		f.errorf(fn.Doc, 1, link(docCommentsLink), category("comments"), T(`comment on exported %s %s should be of the form "%s..."`), kind, name, prefix)
	}
}
...
func (f *file) lintValueSpecDoc(vs *ast.ValueSpec, gd *ast.GenDecl, genDeclMissingComments map[*ast.GenDecl]bool) {
	...
	if len(vs.Names) > 1 {
		...
		for _, n := range vs.Names[1:] {
			if ast.IsExported(n.Name) {
				f.errorf(vs, 1, category("comments"), T("exported %s %s should have its own declaration"), kind, n.Name)
				return
			}
		}
	}
	...
	if vs.Doc == nil && gd.Doc == nil {
		...
		f.errorf(vs, 1, link(docCommentsLink), category("comments"), T("exported %s %s should have comment%s or be unexported"), kind, name, block)
	...
	if !strings.HasPrefix(doc.Text(), prefix) {
		f.errorf(doc, 1, link(docCommentsLink), category("comments"), T(`comment on exported %s %s should be of the form "%s..."`), kind, name, prefix)
	}
}
...
func (f *file) checkStutter(id *ast.Ident, thing string) {
	...
	if next, _ := utf8.DecodeRuneInString(rem); next == '_' || unicode.IsUpper(next) {
		f.errorf(id, 0.8, link(styleGuideBase+"#package-names"), category("naming"), T("%s name will be used as %s.%s by other packages, and that stutters; consider calling this %s"), thing, pkg, name, rem)
	}
}
...
func (f *file) lintVarDecls() {
	...
	f.walk(func(node ast.Node) bool {
		...
		case *ast.ValueSpec:
			...
			if zero {
				f.errorf(rhs, 0.9, category("zero-value"), T("should drop = %s from declaration of var %s; it is the zero value"), f.render(rhs), v.Names[0])
				return false
			}
			...
			f.errorf(v.Type, 0.8, category("type-inference"), T("should omit type %s from declaration of var %s; it will be inferred from the right-hand side"), f.render(v.Type), v.Names[0])
			return false
		}
		return true
	})
...
func (f *file) lintElses() {
	...
	f.walk(func(node ast.Node) bool {
		...
		if _, ok := lastStmt.(*ast.ReturnStmt); ok {
			...
			f.errorf(ifStmt.Else, 1, link(styleGuideBase+"#indent-error-flow"), category("indent"), T("if block ends with a return statement, so drop this else and outdent its block")+extra)
		}
		return true
	})
}
...
func (f *file) lintRanges() {
	f.walk(func(node ast.Node) bool {
		...
		p := f.errorf(rs.Value, 1, category("range-loop"), T("should omit 2nd value from range; this loop is equivalent to `for %s %s range ...`"), f.render(rs.Key), rs.Tok)
...
func (f *file) lintErrorf() {
	f.walk(func(node ast.Node) bool {
		...
		p := f.errorf(node, 1, category("errors"), T("should replace %s(fmt.Sprintf(...)) with %s.Errorf(...)"), f.render(se), errorfPrefix)
	...
	for _, decl := range f.f.Decls {
		...
		for _, spec := range gd.Specs {
			...
			if !strings.HasPrefix(id.Name, prefix) {
				f.errorf(id, 0.9, category("naming"), T("error var %s should have name of the form %sFoo"), id.Name, prefix)
			}
		}
	}
}
...
func (f *file) lintErrorStrings() {
	f.walk(func(node ast.Node) bool {
		...
		f.errorf(str, conf, link(styleGuideBase+"#error-strings"), category("errors"),
			T("error strings should not be capitalized or end with punctuation or a newline"))
		return true
	})
}
...
func (f *file) lintReceiverNames() {
	...
	f.walk(func(n ast.Node) bool {
		...
		if name == "_" {
			f.errorf(n, 1, link(ref), category("naming"), T(`receiver name should not be an underscore`))
			return true
		}
		if name == "this" || name == "self" {
			f.errorf(n, 1, link(ref), category("naming"), T(`receiver name should be a reflection of its identity; don't use generic names such as "this" or "self"`))
			return true
		}
		recv := receiverType(fn)
		if prev, ok := typeReceiver[recv]; ok && prev != name {
			f.errorf(n, 1, link(ref), category("naming"), T("receiver name %s should be consistent with previous receiver name %s for %s"), name, prev, recv)
			return true
		}
...
func (f *file) lintIncDec() {
	f.walk(func(n ast.Node) bool {
		...
		f.errorf(as, 0.8, category("unary-op"), T("should replace %s with %s%s"), f.render(as), f.render(as.Lhs[0]), suffix)
		return true
	})
}
	...
	f.walk(func(n ast.Node) bool {
		...
		for _, r := range ret[:len(ret)-1] {
			if isIdent(r.Type, "error") {
				f.errorf(fn, 0.9, category("arg-order"), T("error should be the last type when returning multiple items"))
				break // only flag one
			}
		}
		return true
	...
	f.walk(func(n ast.Node) bool {
		...
		for _, ret := range fn.Type.Results.List {
			...
			f.errorf(ret.Type, 0.8, category("unexported-type-in-api"),
				T("exported %s %s returns unexported type %s, which can be annoying to use"),
...
func (f *file) lintTimeNames() {
	f.walk(func(node ast.Node) bool {
		...
		for _, name := range v.Names {
			...
			f.errorf(v, 0.9, category("time"), T("var %s is of type %v; don't use unit-specific suffix %q"), name.Name, origTyp, suffix)
		}
		return true
	})
}
...
func (f *file) checkContextKeyType(x *ast.CallExpr) {
	...
	if ktyp, ok := key.Type.(*types.Basic); ok && ktyp.Kind() != types.Invalid {
		f.errorf(x, 1.0, category("context"), fmt.Sprintf(T("should not use basic type %s as key in context.WithValue"), key.Type))
	}
}
	...
	f.walk(func(n ast.Node) bool {
		...
		for _, arg := range fn.Type.Params.List[1:] {
			if isPkgDot(arg.Type, "context", "Context") {
				f.errorf(fn, 0.9, link("https://golang.org/pkg/context/"), category("arg-order"), T("context.Context should be the first parameter of a function"))
				break // only flag one
			}
		}
		return true
